    ...
    if (difficulty === "easy") {
      ...
      for (const word of fromTableWords) {
        ...
        for (const syn of synonyms) {
          if (
            !excludeWords.includes(syn) &&
            !synonymDistractions.includes(syn)
          ) {
      ...
      const shuffledSynonyms = availableSynonyms.sort(
        () => Math.random() - 0.5,
      );
      synonymDistractions.push(
        ...shuffledSynonyms.slice(0, Math.min(count, availableSynonyms.length)),
      );

      const remainingCount = count - synonymDistractions.length;
      if (remainingCount > 0) {
        const randomDistractions = getDistractionWords(remainingCount, [
          ...excludeWords,
          ...synonymDistractions,
        ]);
      ...
      for (const word of fromTableWords) {
        ...
        for (const sim of similarWords) {
          if (
            !excludeWords.includes(sim) &&
            !similarDistractions.includes(sim)
          ) {
      ...
      similarDistractions.push(
        ...shuffledSimilar.slice(0, Math.min(count, availableSimilar.length)),
      );
      ...
      if (remainingCount > 0) {
        ...
        for (const word of fromTableWords) {
          ...
          for (const syn of synonyms) {
            if (
              !excludeWords.includes(syn) &&
              !similarDistractions.includes(syn) &&
              !synonymDistractions.includes(syn)
            ) {
              availableSynonyms.push(syn);
            }
        ...
        synonymDistractions.push(
          ...shuffledSynonyms.slice(
            0,
            Math.min(remainingCount, availableSynonyms.length),
          ),
        );

        const finalRemaining = remainingCount - synonymDistractions.length;
        if (finalRemaining > 0) {
          const randomDistractions = getDistractionWords(finalRemaining, [
            ...excludeWords,
            ...similarDistractions,
            ...synonymDistractions,
          ]);
