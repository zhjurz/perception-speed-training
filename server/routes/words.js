...
router.post("/", (req, res) => {
  try {
    ...
    const insertSimilar = db.prepare(
      "INSERT INTO similar_words (id, word_id, similar_word) VALUES (?, ?, ?)",
    );
...
router.put("/:id", (req, res) => {
  try {
    ...
    const updateWord = db.prepare(
      "UPDATE words SET word = ?, category = ?, is_active = ? WHERE id = ?",
    );
    const deleteSynonyms = db.prepare("DELETE FROM synonyms WHERE word_id = ?");
    const insertSynonym = db.prepare(
      "INSERT INTO synonyms (id, word_id, synonym) VALUES (?, ?, ?)",
    );
    const deleteSimilar = db.prepare(
      "DELETE FROM similar_words WHERE word_id = ?",
    );
    const insertSimilar = db.prepare(
      "INSERT INTO similar_words (id, word_id, similar_word) VALUES (?, ?, ?)",
    );
...
router.post("/batch", (req, res) => {
  try {
    ...
    const insertWord = db.prepare(
      "INSERT OR IGNORE INTO words (id, word, category) VALUES (?, ?, ?)",
    );
...
router.get("/stats/overview", (req, res) => {
  try {
    const totalWords = db
      ...
      .get().count;
    const activeWords = db
      .prepare("SELECT COUNT(*) as count FROM words WHERE is_active = 1")
      .get().count;
    const totalSynonyms = db
      .prepare("SELECT COUNT(*) as count FROM synonyms")
      .get().count;
    const totalSimilar = db
      .prepare("SELECT COUNT(*) as count FROM similar_words")
      .get().count;
